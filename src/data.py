    ...
    def __init__(self, args, tokenizer, filename, collection=None):
        ...
        for i in tqdm(trange(n)):
            ...
            if args.pos_type == "bm25":       
                ...
                if args.is_hard_neg:
                    neg_docs_text = record['ance_pos_docs']
                    neg_docs_pids = record['ance_pos_docs_pids']
            elif args.pos_type == "ance":
                pos_docs_text = record['ance_pos_docs']
                pos_docs_pids = record['ance_pos_docs_pids']
                if args.is_hard_neg:
                    neg_docs_text = record['bm25_pos_docs']
                    neg_docs_pids = record['bm25_pos_docs_pids']
            cur_utt = tokenizer.encode(cur_query, add_special_tokens = True, max_length = args.max_query_length)
            flat_q_concat.extend(cur_utt)   
            for j in range(len(query_context) - 1, -1, -1):
                utt_q = tokenizer.encode(query_context[j], add_special_tokens=True, max_length=args.max_query_length, truncation=True) # not remove [CLS]
            ...
            if args.is_train:
                for pos_doc in pos_docs_text:
                    ...
                    pos_docs, pos_docs_mask = padding_seq_to_same_length(pos_docs, max_pad_length = args.max_concat_length)
                    if args.is_hard_neg:
                        neg_docs.extend(tokenizer.encode(random.choice(neg_docs_text), add_special_tokens=True, max_length=args.max_doc_length, truncation=True))
                        neg_docs, pos_docs_mask = padding_seq_to_same_length(pos_docs, max_pad_length = args.max_concat_length)
...
class Test_Retrieval_cast(Dataset):
    def __init__(self, args, tokenizer, filename, collection=None):
        ...
        for i in tqdm(trange(n)):
            ...
            #if len(record["gpt_rewrite"]) > 0:
            #    rewrite = record["gpt_rewrite"][0]
            #else:
...
class Test_Retrieval_cast21(Dataset):
    def __init__(self, args, tokenizer, filename, collection=None):
        self.examples = []
        ...
        for i in tqdm(trange(n)):
            ...
            #    rewrite = record["gpt_rewrite"][0]
            #else:
            rewrite = record["oracle_utt_text"]
...
class Search_Rel_Passage(Dataset):
    def __init__(self, args, tokenizer, filename):
        ...
        for i in tqdm(trange(n)):
            ...
            conv_query, conv_answer = [], []
            
            for j in range(len(answer_context) - 1, -1, -1):
                utt_q = tokenizer.encode(query_context[j], add_special_tokens=True, max_length=args.max_query_length, truncation=True) # not remove [CLS]
                if len(conv_query) + len(utt_q) > args.max_concat_length // 2:
                    conv_query += utt_q[:args.max_concat_length // 2 - len(conv_query) - 1] + [utt_q[-1]]    # must ended with [SEP]
                    break
                else:
                    conv_query.extend(utt_q) 
                utt_a = tokenizer.encode(answer_context[j], add_special_tokens=True, max_length=args.max_response_length, truncation=True) # not remove [CLS]
                if len(conv_answer) + len(utt_a) > args.max_concat_length // 2:
                    conv_answer += utt_a[:args.max_concat_length // 2 - len(conv_answer) - 1] + [utt_a[-1]]    # must ended with [SEP]
                    break
                else:
                    conv_answer.extend(utt_a) 

            if args.query_type == "q+a":
                search_query.extend(cur_utt)
                search_query.extend(answer)
            elif args.query_type == "convq+conva+topic":
                search_query.extend(conv_query)
                search_query.extend(conv_answer)
